...
#search for select from and make the blocks
blocks = []
from_statement = "from "
    ...
    if select_statement in element[1].lower():
        ...
        for i in sql_list[o:]:
            if from_statement in i[1].lower():
                if len(i[1]) >5:                    #why do we have this len why dont you want to include the from statement?
                    blocks.append(i[0])
                    break
                else:
                    blocks.append(i[0]+1)           #You cut off the block with the assumption there is only one row after the from this does not have to be true
                    break
